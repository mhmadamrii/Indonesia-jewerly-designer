...
export function Trendings({ jewelries }: TrendingsProps) {
  ...
  const [jewelryLimit, setJewelryLimit] = useState(6);
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2">
        {jewelries?.slice(0, jewelryLimit).map((item, idx) => (
          <motion.div
            layoutId={`card-${idx + 1}`}
            ...
            <div className="p-4">
              ...
              <div className="h-[60px]">
                <div className="mb-3 flex flex-wrap gap-1">
                  {item.tags.slice(0, 7).map((tag, idx) => (
      ...
      <div className="flex h-[100px] w-full items-start justify-center">
        {jewelryLimit >= jewelries.length ? (
          <button
